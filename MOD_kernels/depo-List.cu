...
__global__ void kernelfunction_Depo_List(int *mask, int *fd, double *fa, int rows, int cols, double *weights, unsigned int* pkgprogressd, unsigned int* left)
{
...
__global__ void kernelfunction_Depo_ListProgress(int *mask, int *fd, double *fa, int rows, int cols, double *weights, unsigned int* pkgprogressd, unsigned int* left, unsigned int* had, int hadsize)
{
...
int process_Depo_List(Data* data, Data* device, int iter) {
	printf("In process\n");
...
//	checkCudaErrors(cudaMemcpy(progress_h, progress_d. sizeof(unsigned int), cudaMemcpyDeviceToHost));
	...
	kernelfunction_Depo_List<<<dimGrid, dimBlock>>>(device->mask, device->fd, device->fa, gridRows, gridColumns, device->runoffweight, progress_d, list1);
	...
	while (*progress_h > 0) { // while the array still has elements
		...
		*temp = 0;
		...
		kernelfunction_Depo_ListProgress<<<nextBlocks, 128>>>(device->mask, device->fd, device->fa, gridRows, gridColumns, device->runoffweight, progress_d, list1, list2, *progress_h);
...
void Depo_List(Data* data, Data* device, int iter) {
