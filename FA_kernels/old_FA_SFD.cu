...
void accumulateflowSFD(Data* data, Data* device, int iter) {
	...
	//scale_FA(device->mask, device->contribA, device->fa, data->cpucontribA_max, ncell_y, ncell_x, grid1, grid2, 1, BLOCKROWS, BLOCKCOLS, 1);
	...
	printf("Time to complete flow accumulation %.6f s\n", time / 1000.0);
	fprintf(data->outlog, "FA: time to complete flow accumulation %.6f s\n", time / 1000.0);

	cudaMemGetInfo(&freenow, &total);
	fprintf(data->outlog, "FA: Memory on CUDA card free at end of correctflow: %d total: %d\n",freenow/1024,total/1024);
